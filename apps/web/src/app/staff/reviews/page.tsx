...
function Inner() {
  ...
  if (error) return <p style={{ color: 'var(--ink-bad)' }}>{error}</p>;
  ...
  return (
    <section>
      <div style={{ background: 'var(--c1e293b)', border: '1px solid var(--c334155)', borderRadius: 16, padding: 22, textAlign: 'center', marginBottom: 18 }}>
        ...
        <p style={{ color: 'var(--ccbd5e1)', fontSize: 14, margin: '14px 0 4px', fontWeight: 600 }}>Ask your client to scan this after their service</p>
        <p style={{ color: 'var(--c64748b)', fontSize: 12, margin: 0 }}>They rate you (you earn points), then can post on Google.</p>

        <button onClick={copy} style={{ marginTop: 14, padding: '10px 16px', borderRadius: 10, border: '1px solid var(--c475569)', background: copied ? '#16a34a' : 'transparent', color: 'var(--ce2e8f0)', fontSize: 14, cursor: 'pointer' }}>
      ...
      {data.recent.length === 0 ? (
        <p style={{ color: 'var(--c94a3b8)', fontSize: 14 }}>No feedback yet — start asking your clients!</p>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {data.recent.map((r) => (
            <div key={r.id} style={{ background: 'var(--c1e293b)', border: '1px solid var(--c334155)', borderRadius: 10, padding: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--ink-warn)' }}>{'★'.repeat(r.rating)}<span style={{ color: 'var(--ink-faint)' }}>{'★'.repeat(5 - r.rating)}</span></span>
