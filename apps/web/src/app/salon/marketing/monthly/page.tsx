...
function openPrint(data: Monthly | null, c: Content, vi: boolean, money: (n: number) => string, salonName = '') {
  ...
  const bignum = (val: string, label: string, dl?: Delta, green?: boolean) =>
    `<div style="flex:1;min-width:78px;text-align:center"><div style="font-size:30px;font-weight:800;color:${green ? '#059669' : '#111827'};line-height:1.15">${val}</div><div style="font-size:14.5px;color:#6b7280;margin-top:2px">${label}</div><div style="font-size:14px">${arrow(dl)}</div></div>`;
  ...
  const socHtml = (data.socialInsights ?? []).map((si) => {
    ...
    const st = (label: string, val: number | null, dl?: SocialDelta | null) => (val == null ? '' : `<div style="text-align:center;flex:1;min-width:58px"><div style="font-size:18.6px;font-weight:800">${f(val)}${ar(dl)}</div><div style="font-size:13.5px;color:#6b7280">${label}</div></div>`);
  ...
  const growth = `<div style="display:flex;gap:16px">
    <div style="flex:1"><div style="color:#1877f2;font-weight:800;font-size:15.4px">Facebook</div><div style="font-size:24.2px;font-weight:800;color:#0f2a52">${fnum(fb?.followers)} ${arS(fb?.vsPrev?.followers)}</div><div style="font-size:13.5px;color:#94a3b8">${t('Tổng theo dõi', 'Total followers')}</div>${cumF.length > 1 ? sparkD(cumF, '#1877f2') : `<div style="font-size:13.5px;color:#94a3b8;margin-top:10px;padding:12px;background:#f6f8fc;border:1px dashed #d8e0ee;border-radius:8px;text-align:center">${t('Biểu đồ tăng trưởng theo tháng hiện khi có ≥2 tháng đồng bộ','Monthly growth chart appears once ≥2 months are synced')}</div>`}</div>
    <div style="flex:1"><div style="color:#e1306c;font-weight:800;font-size:15.4px">Instagram</div><div style="font-size:24.2px;font-weight:800;color:#0f2a52">${fnum(ig?.followers)} ${arS(ig?.vsPrev?.followers)}</div><div style="font-size:13.5px;color:#94a3b8">${t('Tổng theo dõi', 'Total followers')}</div>${cumA.length > 1 ? sparkD(cumA, '#e1306c') : `<div style="font-size:13.5px;color:#94a3b8;margin-top:10px;padding:12px;background:#f6f8fc;border:1px dashed #d8e0ee;border-radius:8px;text-align:center">${t('Biểu đồ tăng trưởng theo tháng hiện khi có ≥2 tháng đồng bộ','Monthly growth chart appears once ≥2 months are synced')}</div>`}</div>
  </div>`;
  const igP = ig?.posts ?? [];
  ...
  if (g && (g.impressions != null || g.calls != null || g.directions != null || g.websiteClicks != null || g.bookings != null)) {
    ...
    const card = (label: string, sub: string, val: number | null | undefined, d: SocialDelta | null | undefined, color: string) =>
      `<div style="flex:1;min-width:150px;background:#fff;border:1px solid #e6ebf3;border-radius:12px;padding:11px 13px"><div style="font-size:14.1px;color:#475569;font-weight:600">${esc(label)}</div><div style="font-size:13.5px;color:#94a3b8">${esc(sub)}</div><div style="display:flex;align-items:baseline;gap:8px;margin-top:5px"><div style="font-size:23.1px;font-weight:800;color:${color}">${fnum(val)}</div>${arS(d)}</div></div>`;
    ...
    const reviewPanel = rv && (rv.rating != null || rv.count != null)
      ? `<div style="display:flex;align-items:center;gap:8px"><div style="font-size:31.8px;font-weight:800;color:#0f2a52">${rv.rating ?? '—'}</div><div><div style="color:#f59e0b;font-size:17.4px;line-height:1">${stars(rv.rating || 0)}</div><div style="font-size:13.5px;color:#64748b">${fnum(rv.count)} ${t('đánh giá', 'reviews')}</div></div></div>
        <div style="display:flex;gap:18px;margin-top:10px;border-top:1px solid #eef1f6;padding-top:8px">
          <div><div style="font-size:20.9px;font-weight:800;color:#16A34A">${rv.newThisMonth != null ? '+' + fnum(rv.newThisMonth) : '—'}</div><div style="font-size:13.5px;color:#64748b">${t('review mới tháng này', 'new this month')}</div></div>
          <div><div style="font-size:20.9px;font-weight:800;color:${(rv.badCount || 0) > 0 ? '#DC2626' : '#0f2a52'}">${rv.badCount != null ? fnum(rv.badCount) : '—'}</div><div style="font-size:13.5px;color:#64748b">${t('review xấu (≤2★)', 'bad (≤2★)')}</div></div>
  ...
  *{box-sizing:border-box} body{font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#0f2a52;margin:0 auto;padding:0;background:#eef2f8;width:281mm}
  ...
  <div style="background:linear-gradient(120deg,#0f2a52,#1e3a8a);color:#fff;border-radius:14px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center;gap:14px">
    <div>
      ...
      <div style="font-size:15px;opacity:.85;margin-top:3px">${t('Báo cáo Marketing tổng hợp · Tháng', 'Marketing report · Month')} ${data.month} · ${t('bởi Lumio Agency', 'by Lumio Agency')}</div>
    </div>
