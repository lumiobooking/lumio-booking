...
export class MessengerService implements OnModuleInit {
  ...
  /**
   * Hand the unread mark back.
   *
   * `readAt: null` rather than a date in the past: unread is computed as
   * "readAt is missing, or older than the last message", so null is the one
   * value that stays unread no matter what arrives next. Backdating would be
   * undone by the next inbound message re-stamping lastMessageAt.
   */
  async markThreadUnread(user: AuthenticatedUser, id: string) {
    const tenantId = this.tenantId(user);
    const row = await this.prisma.messengerThread.findFirst({ where: { id, tenantId } });
    if (!row) throw new NotFoundException('Thread not found');
    await this.prisma.messengerThread.update({ where: { id: row.id }, data: { readAt: null } as never });
    return { ok: true };
  }

  /**
   * Mark every conversation in this salon read.
   *
   * Scoped by tenantId like every other query here — a salon can only ever
   * clear its own pile. `updateMany` and not a loop: this is one statement
   * against rows the caller already owns.
   */
  async markAllRead(user: AuthenticatedUser) {
    const tenantId = this.tenantId(user);
    const r = await this.prisma.messengerThread.updateMany({
      where: { tenantId },
...
${infoBlock ? infoBlock + '\n' : ''}Only state hours, prices, services, address, and contact info that are given to you here; never invent them. Do not book or promise a time outside business hours — if the customer asks for a closed day or time, tell them the salon is closed then and offer the nearest open time. If the customer is upset or asks for a human, tell them a staff member will follow up soon. Do not ask for payment.${aiInstruction ? `\nSalon owner's extra notes: ${aiInstruction}` : ''}`;
    ...
    const bookingTools = [
      ...
      },
      {
        ...
        description: 'Cancel an existing appointment. ONLY call this after the customer has clearly confirmed — repeat the day and time back and get a yes first, because this empties the salon’s chair and you cannot undo it from the chat. The tool checks the salon’s notice policy and returns the exact sentence to say, including when it refuses.',
        input_schema: {
          type: 'object',
          properties: {
            appointmentId: { type: 'string', description: 'The id returned by find_appointment. Never guessed.' },
          ...
          required: ['appointmentId', 'customerPhone'],
        },
      },
      {
        name: 'save_birthday',
...
- Asked what makes you different / what is special / how you compare to other agencies, booking software or hiring someone: answer in TWO short lines and lead with the OUTCOME, never the machinery.
  ...
  Line 2 — one concrete proof of that, chosen to fit what they already told you: found on Maps → messages answered in seconds → the appointment lands in the calendar by itself; and afterwards the reminders and the win-backs run on their own.
  Then step 3. Never list features, never explain how it works internally, never use words like "chốt lịch thẳng vào hệ thống", "không cần nhân viên trực page", "POS", "webhook" — an owner buys more customers and fewer no-shows, not plumbing. If they ask HOW, then you may explain, briefly.
  ...
  private async runTool(
    ...
    try {
      ...
      if (name === 'cancel_appointment') {
        const id = String(input.appointmentId || '').trim();
        const phone = String(input.customerPhone || '').trim();
        if (!id || !phone) return 'Missing the appointment id or the phone; ask for what is missing.';
        const r = await this.bookings.selfCancel({ tenantId, appointmentId: id, phone, by: 'messenger' });
        if (!r.ok) {
