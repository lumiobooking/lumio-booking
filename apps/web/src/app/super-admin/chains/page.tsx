...
const card: React.CSSProperties = { background: 'var(--c111827)', border: '1px solid var(--c1f2937)', borderRadius: 12, padding: 18 };
const input: React.CSSProperties = { padding: '9px 11px', borderRadius: 8, border: '1px solid var(--c334155)', background: 'var(--c0b1120)', color: 'var(--ce2e8f0)', fontSize: 14 };
...
const ghost: React.CSSProperties = { padding: '7px 11px', borderRadius: 8, border: '1px solid var(--c334155)', background: 'transparent', color: 'var(--ce2e8f0)', fontSize: 13, cursor: 'pointer' };
...
export default function ChainsPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--c0b1120)', color: 'var(--ce2e8f0)', padding: '28px 24px' }}>
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        {err && <div style={{ background: 'var(--c7f1d1d)', color: 'var(--cfecaca)', padding: '10px 14px', borderRadius: 8, marginBottom: 14, fontSize: 14 }}>{err}</div>}

        <form onSubmit={createGroup} style={{ ...card, display: 'flex', gap: 10, alignItems: 'flex-end', marginBottom: 18 }}>
          <label style={{ flex: 1 }}>
            <span style={{ display: 'block', fontSize: 12, color: 'var(--c94a3b8)', marginBottom: 5 }}>Tên chuỗi mới</span>
        ...
        {loading ? <p style={{ color: 'var(--c94a3b8)' }}>Đang tải…</p> : groups.length === 0 ? (
          <p style={{ color: 'var(--c64748b)' }}>Chưa có chuỗi nào. Tạo một chuỗi ở trên để bắt đầu.</p>
        ) : groups.map((g) => (
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
              {g.tenants.map((t) => (
                <span key={t.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'var(--c1e293b)', border: '1px solid var(--c334155)', borderRadius: 999, padding: '5px 6px 5px 12px', fontSize: 13 }}>
                  {t.name}
                  <button onClick={() => removeTenant(t.id)} title="Gỡ khỏi chuỗi" style={{ background: 'none', border: 'none', color: 'var(--cf87171)', cursor: 'pointer', fontSize: 14 }}>✕</button>
                </span>
              ))}
              {g.tenants.length === 0 && <span style={{ color: 'var(--c64748b)', fontSize: 13 }}>Chưa có chi nhánh.</span>}
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
              {g.users.map((u) => (
                <span key={u.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'var(--c312e81)', border: '1px solid #4f46e5', borderRadius: 999, padding: '5px 6px 5px 12px', fontSize: 13 }}>
                  {u.email}
                  <button onClick={() => unlinkUser(g.id, u.id)} title="Bỏ liên kết" style={{ background: 'none', border: 'none', color: 'var(--cfca5a5)', cursor: 'pointer', fontSize: 14 }}>✕</button>
                </span>
              ))}
              {g.users.length === 0 && <span style={{ color: 'var(--c64748b)', fontSize: 13 }}>Chưa liên kết chủ nào.</span>}
            </div>
