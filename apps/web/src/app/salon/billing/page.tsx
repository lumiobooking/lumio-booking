    ...
    <section style={{ maxWidth: 760 }}>
      ...
      <div style={{ ...ui.card, marginBottom: 18 }}>
        ...
        <div style={{ borderTop: '1px solid #334155', marginTop: 14, paddingTop: 12, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12 }}>
          {sum?.subscription ? (
            <>
              <Detail label="Status" value={statusLabel(sum.subscription.status)} />
              <Detail label="Billing cycle" value={sum.subscription.interval === 'YEARLY' ? 'Yearly' : 'Monthly'} />
              <Detail label="Registered on" value={fmtDate(sum.subscription.createdAt)} />
              {sum.subscription.trialEndsAt && <Detail label="Free trial ends" value={fmtDate(sum.subscription.trialEndsAt)} />}
              <Detail label={sum.subscription.status === 'CANCELLED' ? 'Ends on' : 'Renews on'} value={fmtDate(sum.subscription.currentPeriodEnd)} />
            </>
          ) : sum?.billingExempt ? (
            <Detail label="Access" value="Free — no expiry" />
          ) : sum?.accessUntil ? (
            <>
              <Detail label="Access type" value="Granted by admin" />
              <Detail label="Valid until" value={fmtDate(sum.accessUntil)} />
            </>
          ) : (
            <Detail label="Subscription" value="None yet — pick a plan to start" />
          )}
        </div>
      </div>
...
function statusLabel(s: string): string {
  const map: Record<string, string> = { TRIALING: 'Free trial', ACTIVE: 'Active', PAST_DUE: 'Payment due', CANCELLED: 'Cancelled' };
  return map[s] ?? s;
}
