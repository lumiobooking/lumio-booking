...
function openPrint(data: Monthly | null, c: Content, vi: boolean, money: (n: number) => string, salonName = '') {
  ...
  const arrow = (dl?: Delta) => {
    ...
    const up = dl.pct >= 0;
    return `<span style="color:${up ? '#059669' : '#dc2626'};font-size:15.4px;font-weight:700">${up ? '▲' : '▼'} ${Math.abs(dl.pct)}%</span>`;
  };
  const bignum = (val: string, label: string, dl?: Delta, green?: boolean) =>
    `<div style="flex:1;min-width:78px;text-align:center"><div style="font-size:25.4px;font-weight:800;color:${green ? '#059669' : '#111827'}">${val}</div><div style="font-size:14.1px;color:#6b7280">${label}</div><div style="font-size:14.1px">${arrow(dl)}</div></div>`;
  ...
  const socHtml = (data.socialInsights ?? []).map((si) => {
    ...
    const ar = (dl?: SocialDelta | null) => (dl && dl.pct != null ? `<span style="color:${dl.pct >= 0 ? '#059669' : '#dc2626'};font-size:12.8px;font-weight:700"> ${dl.pct >= 0 ? '▲' : '▼'}${Math.abs(dl.pct)}%</span>` : '');
    const st = (label: string, val: number | null, dl?: SocialDelta | null) => (val == null ? '' : `<div style="text-align:center;flex:1;min-width:58px"><div style="font-size:18.6px;font-weight:800">${f(val)}${ar(dl)}</div><div style="font-size:12.8px;color:#6b7280">${label}</div></div>`);
  ...
  const postsHtml = igPosts.map((p) => {
    ...
    return `<div style="display:flex;gap:9px;align-items:center;padding:6px 0;border-top:1px solid #f0ece7">${thumb}<div style="flex:1;min-width:0"><div style="font-size:14.1px;color:#6b7280">${esc(tl)} · ${dt}${p.caption ? ' · ' + esc(p.caption) : ''}</div><div style="font-size:15.4px;margin-top:2px">${stats}</div></div></div>`;
  }).join('');
  ...
  if (igAud && (igAud.gender || igAud.age)) {
    ...
    const ageRows = ['13-17', '18-24', '25-34', '35-44', '45-54', '55-64', '65+'].filter((k) => age[k] != null).map((k) => { const pct = Math.round((age[k] / at) * 1000) / 10; return `<div style="display:flex;align-items:center;gap:8px;margin:3px 0"><span style="width:44px;font-size:14.1px;color:#6b7280">${k}</span><span style="flex:1;height:8px;background:#eee;border-radius:4px;overflow:hidden"><span style="display:block;height:100%;width:${pct}%;background:#6366f1"></span></span><span style="width:38px;font-size:14.1px;text-align:right">${pct}%</span></div>`; }).join('');
    audienceHtml = `${gRows ? `<div style="font-size:15.4px;color:#374151;margin-bottom:6px"><b>${t('Giới tính', 'Gender')}</b> — ${gRows}</div>` : ''}${ageRows ? `<div class="lbl" style="margin-bottom:2px">${t('Độ tuổi', 'Age')}</div>${ageRows}` : ''}`;
  }
  ...
  const arS = (dl?: SocialDelta | null) => (dl && dl.pct != null ? `<span style="color:${dl.pct >= 0 ? '#16a34a' : '#dc2626'};font-weight:700;font-size:13.4px">${dl.pct >= 0 ? '▲' : '▼'}${Math.abs(dl.pct)}%</span>` : '');
  ...
  const panel = (title: string, inner: string) => `<div style="background:#fff;border:1px solid #e6e9f0;border-radius:12px;padding:12px 14px"><div style="font-weight:800;font-size:15.4px;color:#0B1F3A;text-transform:uppercase;letter-spacing:.6px;border-bottom:2px solid #E4EAF2;padding-bottom:5px;margin-bottom:9px">${title}</div>${inner}</div>`;
  const prow = (label: string, fv: string, fd: SocialDelta | null | undefined, iv: string, idv: SocialDelta | null | undefined, tv?: string, tdv?: SocialDelta | null) =>
    `<tr><td style="padding:5px 2px;color:#475569;font-size:14.7px">${label}</td><td style="padding:5px 2px;text-align:right;font-weight:700;color:#0f2a52">${fv} ${arS(fd)}</td><td style="padding:5px 2px;text-align:right;font-weight:700;color:#0f2a52">${iv} ${arS(idv)}</td>${tt ? `<td style="padding:5px 2px;text-align:right;font-weight:700;color:#0f2a52">${tv ?? '—'} ${arS(tdv)}</td>` : ''}</tr>`;
  const perfTable = `<table style="width:100%;border-collapse:collapse">
    <tr style="font-size:14.1px"><td></td><td style="text-align:right;padding-bottom:4px"><span style="color:#1877f2;font-weight:800">Facebook</span></td><td style="text-align:right;padding-bottom:4px"><span style="color:#e1306c;font-weight:800">Instagram</span></td>${tt ? '<td style="text-align:right;padding-bottom:4px"><span style="color:#010101;font-weight:800">TikTok</span></td>' : ''}</tr>
    ...
    ${prow(t('Người tiếp cận (Reach)', 'Reach'), fb?.reach == null ? ('<span style="font-size:12.5px;color:#94a3b8;font-weight:600">' + t('Meta ngừng cung cấp', 'retired by Meta') + '</span>') : fnum(fb?.reach), fb?.vsPrev?.reach, fnum(ig?.reach), ig?.vsPrev?.reach, '<span style="font-size:12.5px;color:#94a3b8;font-weight:600">' + t('Không áp dụng', 'n/a') + '</span>', null)}
  ...
  const growth = `<div style="display:flex;gap:16px">
    <div style="flex:1"><div style="color:#1877f2;font-weight:800;font-size:15.4px">Facebook</div><div style="font-size:24.2px;font-weight:800;color:#0f2a52">${fnum(fb?.followers)} ${arS(fb?.vsPrev?.followers)}</div><div style="font-size:13.4px;color:#94a3b8">${t('Tổng theo dõi', 'Total followers')}</div>${cumF.length > 1 ? sparkD(cumF, '#1877f2') : `<div style="font-size:12.8px;color:#94a3b8;margin-top:10px;padding:12px;background:#f6f8fc;border:1px dashed #d8e0ee;border-radius:8px;text-align:center">${t('Biểu đồ tăng trưởng theo tháng hiện khi có ≥2 tháng đồng bộ','Monthly growth chart appears once ≥2 months are synced')}</div>`}</div>
    <div style="flex:1"><div style="color:#e1306c;font-weight:800;font-size:15.4px">Instagram</div><div style="font-size:24.2px;font-weight:800;color:#0f2a52">${fnum(ig?.followers)} ${arS(ig?.vsPrev?.followers)}</div><div style="font-size:13.4px;color:#94a3b8">${t('Tổng theo dõi', 'Total followers')}</div>${cumA.length > 1 ? sparkD(cumA, '#e1306c') : `<div style="font-size:12.8px;color:#94a3b8;margin-top:10px;padding:12px;background:#f6f8fc;border:1px dashed #d8e0ee;border-radius:8px;text-align:center">${t('Biểu đồ tăng trưởng theo tháng hiện khi có ≥2 tháng đồng bộ','Monthly growth chart appears once ≥2 months are synced')}</div>`}</div>
  </div>`;
  const igP = ig?.posts ?? [];
  ...
  const contentTable = `<table style="width:100%;border-collapse:collapse;font-size:14.7px">
    <tr style="font-size:13.4px;color:#94a3b8"><td></td><td style="text-align:right"><b style="color:#1877f2">FB</b></td><td style="text-align:right"><b style="color:#e1306c">IG</b></td></tr>
  ...
  const top3 = topAll.map((p) => { const th = p.thumbnail && p.thumbnail.startsWith('http') ? `<img src="${esc(p.thumbnail)}" style="width:34px;height:34px;border-radius:6px;object-fit:cover;flex-shrink:0"/>` : `<div style="width:34px;height:34px;border-radius:6px;background:${p._col}22;color:${p._col};flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:12.5px;font-weight:800">${p._pf}</div>`; const badge = `<span style="font-size:12.5px;font-weight:800;color:#fff;background:${p._col};border-radius:4px;padding:1px 5px;margin-right:5px">${p._pf}</span>`; return `<div style="display:flex;gap:8px;align-items:center;margin:5px 0">${th}<div style="flex:1;min-width:0"><div style="font-size:13.4px;color:#475569;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${badge}${esc(p.caption || (p.type === 'reel' ? 'Reel' : 'Post'))}</div><div style="font-size:13.4px;color:#0f2a52"><b>${fnum(p.views)}</b> ${t('xem', 'views')}${p.reach != null ? ` · <b>${fnum(p.reach)}</b> reach` : ''} · <b>${fnum((p.likes ?? 0) + (p.comments ?? 0))}</b> ${t('tương tác', 'eng')}</div></div></div>`; }).join('') || `<div style="color:#94a3b8;font-size:14.1px">—</div>`;
  const contentPanel = `<div style="display:flex;gap:14px"><div style="flex:0 0 40%">${contentTable}</div><div style="flex:1;min-width:0;border-left:1px solid #eef1f6;padding-left:12px"><div style="font-size:13.4px;color:#94a3b8;margin-bottom:2px">${t('TOP BÀI (FB + IG) — theo lượt xem', 'TOP POSTS (FB + IG) — by views')}</div>${top3}</div></div>`;
  const adsPanel = spendLine
    ? `<div style="font-size:15.4px;color:#0f2a52">${t('Tổng chi', 'Spend')}: <b>${money(total)}</b><div style="font-size:14.1px;color:#6b7280;margin-top:4px">${spendLine}</div></div>`
    : `<div style="font-size:14.7px;color:#94a3b8;line-height:1.6">${t('Chưa chạy quảng cáo tháng này. Khi bật quảng cáo, mục này hiển thị Chi phí · Reach · Click · CPC · CTR · ROAS.', 'No paid ads this month. Once ads run, this shows Spend · Reach · Clicks · CPC · CTR · ROAS.')}</div>`;
  let donutHtml = '';
  if (igAud?.gender) { const g = igAud.gender; const gt = Object.values(g).reduce((a, b2) => a + b2, 0) || 1; let acc = 0; const seg = ([['F', '#e1306c'], ['M', '#3b82f6'], ['U', '#cbd5e1']] as [string, string][]).map(([k, cc]) => { const pctv = Math.round(((g[k] || 0) / gt) * 1000) / 10; const from = acc; acc += pctv; return `${cc} ${from}% ${acc}%`; }).join(','); const leg = ([['F', t('Nữ', 'Female'), '#e1306c'], ['M', t('Nam', 'Male'), '#3b82f6'], ['U', t('Khác', 'Other'), '#cbd5e1']] as [string, string, string][]).filter(([k]) => g[k] != null).map(([k, lb, cc]) => `<div style="display:flex;align-items:center;gap:6px;font-size:14.1px;margin:2px 0"><span style="width:9px;height:9px;border-radius:2px;background:${cc}"></span><span style="color:#475569">${esc(lb)}</span><b style="margin-left:auto;color:#0f2a52">${Math.round(((g[k] || 0) / gt) * 1000) / 10}%</b></div>`).join(''); donutHtml = `<div style="display:flex;gap:12px;align-items:center"><div style="width:76px;height:76px;border-radius:50%;flex-shrink:0;background:conic-gradient(${seg});-webkit-mask:radial-gradient(circle 23px at center,transparent 98%,#000 100%);mask:radial-gradient(circle 23px at center,transparent 98%,#000 100%)"></div><div style="flex:1">${leg}</div></div>`; }
  const audiencePanel = (donutHtml || audienceHtml)
    ? `${donutHtml}${igAud?.age ? `<div style="margin-top:8px">${(() => { const age = igAud.age || {}; const at = Object.values(age).reduce((a, b2) => a + b2, 0) || 1; return ['13-17', '18-24', '25-34', '35-44', '45-54', '55-64', '65+'].filter((k) => age[k] != null).map((k) => { const pv = Math.round(((age[k] || 0) / at) * 1000) / 10; return `<div style="display:flex;align-items:center;gap:8px;margin:2px 0"><span style="width:42px;font-size:13.4px;color:#94a3b8">${k}</span><span style="flex:1;height:7px;background:#eef1f6;border-radius:4px;overflow:hidden"><span style="display:block;height:100%;width:${pv}%;background:#6366f1"></span></span><span style="width:36px;font-size:13.4px;text-align:right;color:#0f2a52">${pv}%</span></div>`; }).join(''); })()}</div>` : ''}`
    : `<div style="font-size:14.1px;color:#94a3b8">${t('Meta không cung cấp nhân khẩu cho Facebook. Instagram cần ≥100 follower.', 'Meta does not provide Facebook demographics. Instagram needs ≥100 followers.')}</div>`;
  const posBox = hiHtml ? `<div style="font-size:14.7px;line-height:1.55;color:#166534">${hiHtml}</div>` : `<div style="font-size:14.1px;color:#94a3b8">—</div>`;
  const negBox = issHtml ? `<div style="font-size:14.7px;line-height:1.55;color:#b45309">${issHtml}</div>` : `<div style="font-size:14.1px;color:#94a3b8">—</div>`;
  const insightBox = (c.tldr && L(c.tldr)) || (c.summary && L(c.summary)) ? `<div style="font-size:14.7px;color:#334155;line-height:1.6">${esc(L(c.tldr) || L(c.summary))}</div>` : `<div style="font-size:14.1px;color:#94a3b8">—</div>`;
  const insightsBox = (c.insights ?? []).length ? (c.insights ?? []).map((x) => `<div style="font-size:14.7px;margin:3px 0;color:#334155">• ${esc(L(x))}</div>`).join('') : insightBox;
  const planGroup = (label: string, items?: Item[]) => `<div style="flex:1;min-width:130px"><div style="font-size:14.1px;font-weight:800;color:#4338ca;margin-bottom:3px">${esc(label)}</div>${(items ?? []).length ? (items ?? []).map((x) => `<div style="font-size:14.1px;margin:2px 0;color:#334155">• ${esc(L(x))}</div>`).join('') : '<div style="font-size:14.1px;color:#94a3b8">—</div>'}</div>`;
  ...
  const kehoachHtml = nm && (nm.content || nm.ads || nm.growth || nm.kpi)
    ...
    : `<div style="font-size:14.7px;color:#4338ca;line-height:1.55">${plan}</div>`;
  ...
  if (g && (g.impressions != null || g.calls != null || g.directions != null || g.websiteClicks != null || g.bookings != null)) {
    ...
    const card = (label: string, sub: string, val: number | null | undefined, d: SocialDelta | null | undefined, color: string) =>
      `<div style="flex:1;min-width:150px;background:#fff;border:1px solid #e6ebf3;border-radius:12px;padding:11px 13px"><div style="font-size:14.1px;color:#475569;font-weight:600">${esc(label)}</div><div style="font-size:12.5px;color:#94a3b8">${esc(sub)}</div><div style="display:flex;align-items:baseline;gap:8px;margin-top:5px"><div style="font-size:23.1px;font-weight:800;color:${color}">${fnum(val)}</div>${arS(d)}</div></div>`;
    ...
    const kwPanel = kw.length ? `<div style="font-size:14.1px">${kw.map((k) => `<div style="display:flex;justify-content:space-between;margin:2px 0;color:#475569"><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(k.keyword)}</span><b style="color:#0f2a52;flex-shrink:0;margin-left:8px">${fnum(k.count)}</b></div>`).join('')}</div>` : `<div style="font-size:14.1px;color:#94a3b8">${t('Chưa có dữ liệu từ khoá tháng này.', 'No keyword data this month.')}</div>`;
    const impSeries = (g.series || []).map((x) => x.impressions || 0);
    const trendPanel = impSeries.length > 1 ? sparkD(impSeries, '#1a73e8') : `<div style="font-size:14.1px;color:#94a3b8">${t('Cần ≥2 tháng đồng bộ để vẽ xu hướng.', 'Need ≥2 synced months for a trend.')}</div>`;
    ...
    const kpiRow = (label: string, cur: number | null | undefined) => (cur != null ? `<div style="display:flex;justify-content:space-between;font-size:14.7px;margin:3px 0"><span style="color:#475569">${esc(label)}</span><b style="color:#0f2a52">≥ ${fnum(up(cur))}</b></div>` : '');
    const kpiPanel = `${kpiRow(t('Lượt xem hồ sơ', 'Profile views'), g.impressions)}${kpiRow(t('Lượt gọi', 'Calls'), g.calls)}${kpiRow(t('Lượt chỉ đường', 'Directions'), g.directions)}${kpiRow(t('Lượt truy cập web', 'Website'), g.websiteClicks)}${kpiRow(t('Lượt đặt lịch', 'Bookings'), g.bookings)}` || '<div style="font-size:14.1px;color:#94a3b8">—</div>';
    const rv = g.reviews;
    ...
    const reviewPanel = rv && (rv.rating != null || rv.count != null)
      ? `<div style="display:flex;align-items:center;gap:8px"><div style="font-size:31.8px;font-weight:800;color:#0f2a52">${rv.rating ?? '—'}</div><div><div style="color:#f59e0b;font-size:17.4px;line-height:1">${stars(rv.rating || 0)}</div><div style="font-size:13.4px;color:#64748b">${fnum(rv.count)} ${t('đánh giá', 'reviews')}</div></div></div>
        <div style="display:flex;gap:18px;margin-top:10px;border-top:1px solid #eef1f6;padding-top:8px">
          <div><div style="font-size:20.9px;font-weight:800;color:#16A34A">${rv.newThisMonth != null ? '+' + fnum(rv.newThisMonth) : '—'}</div><div style="font-size:12.8px;color:#64748b">${t('review mới tháng này', 'new this month')}</div></div>
          <div><div style="font-size:20.9px;font-weight:800;color:${(rv.badCount || 0) > 0 ? '#DC2626' : '#0f2a52'}">${rv.badCount != null ? fnum(rv.badCount) : '—'}</div><div style="font-size:12.8px;color:#64748b">${t('review xấu (≤2★)', 'bad (≤2★)')}</div></div>
        </div>${(rv.recent || []).slice(0, 2).map((r) => `<div style="border-top:1px solid #eef1f6;padding:5px 0;font-size:13.4px;margin-top:4px"><b>${esc(r.author || '')}</b> · <span style="color:#f59e0b">${'★'.repeat(r.rating || 0)}</span><div style="color:#475569">${esc((r.comment || '').slice(0, 90))}</div></div>`).join('')}`
      : `<div style="font-size:14.1px;color:#94a3b8;line-height:1.6">${t('Chưa có dữ liệu đánh giá — vào Chỉnh sửa → “Google — Đánh giá (nhập tay)” để nhập rating, tổng review, review mới, review xấu.', 'No review data — go to Edit → “Google — Reviews (manual)” to enter rating, totals, new & bad reviews.')}</div>`;
    const recs: string[] = [];
    ...
    const recPanel = `<div style="font-size:14.1px;line-height:1.5">${recs.map((r) => `<div style="margin:3px 0;color:#334155">• ${esc(r)}</div>`).join('')}</div>`;
    ...
    <div style="background:linear-gradient(120deg,#1a73e8,#174ea6);color:#fff;border-radius:14px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center">
      <div><div style="display:inline-block;background:rgba(255,255,255,.16);border-radius:6px;padding:3px 10px;font-size:13.4px;font-weight:700;letter-spacing:1px">${t('BÁO CÁO CUỐI THÁNG', 'MONTHLY REPORT')}</div>
      <div style="font-size:25.4px;font-weight:800;margin-top:6px">GOOGLE BUSINESS PROFILE</div>
      <div style="font-size:15.4px;opacity:.85;margin-top:2px">${t('Tháng', 'Month')} ${data.month} · ${t('bởi Lumio Agency', 'by Lumio Agency')}</div></div>
      <span style="width:46px;height:46px;border-radius:12px;background:#fff;display:grid;place-items:center;font-weight:800;font-size:24.2px;color:#1a73e8">G</span>
    </div>
  ...
  *{box-sizing:border-box} body{font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#0f2a52;margin:0 auto;padding:0;background:#eef2f8;width:281mm}
  ...
  <div style="background:linear-gradient(120deg,#0f2a52,#1e3a8a);color:#fff;border-radius:14px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center;gap:14px">
    <div>
      <div style="display:inline-block;background:rgba(255,255,255,.16);border-radius:6px;padding:3px 10px;font-size:13.4px;font-weight:700;letter-spacing:1px">${t('BÁO CÁO CUỐI THÁNG', 'MONTHLY REPORT')}</div>
      <div style="font-size:27.6px;font-weight:800;letter-spacing:.5px;margin-top:6px">${esc(salonName || t('Báo cáo Marketing', 'Marketing Report'))}</div>
      <div style="font-size:15.4px;opacity:.85;margin-top:2px">${t('Báo cáo Marketing tổng hợp · Tháng', 'Marketing report · Month')} ${data.month} · ${t('bởi Lumio Agency', 'by Lumio Agency')}</div>
    </div>
    <div style="text-align:right;font-size:12.5px;opacity:.8"><div style="font-size:17.4px;font-weight:800">Lumio Agency</div><div style="margin-top:2px">Facebook · Instagram · TikTok · Google</div></div>
  </div>
  ...
  <div style="font-size:12.8px;color:#94a3b8;margin-top:10px;text-align:center">${t('Số liệu organic lấy trực tiếp từ Facebook/Instagram · Meta đã ngừng một số chỉ số Facebook · AI tổng hợp, Lumio duyệt trước khi gửi.', 'Organic data pulled directly from Facebook/Instagram · Meta discontinued some Facebook metrics · AI-summarised, reviewed by Lumio.')}</div>
  </div>
  ${gbpHtml}
  <div class="page">
  <div style="background:#0f2a52;color:#fff;border-radius:10px;padding:10px 15px;font-size:16.2px;font-weight:800">${t('TỔNG KẾT & KẾ HOẠCH THÁNG TỚI — TẤT CẢ KÊNH', 'SUMMARY & NEXT-MONTH PLAN — ALL CHANNELS')}</div>
  <div class="grid" style="grid-template-columns:1fr 1fr;margin-top:10px">
    ${panel('◆ ' + t('ĐÁNH GIÁ CHUNG', 'OVERALL ASSESSMENT'), `<div style="font-size:14.1px;font-weight:700;color:#166534;margin-bottom:2px">${t('Điểm tích cực', 'Wins')}</div>${posBox}<div style="font-size:14.1px;font-weight:700;color:#b45309;margin:8px 0 2px">${t('Điểm cần cải thiện', 'To improve')}</div>${negBox}`)}
