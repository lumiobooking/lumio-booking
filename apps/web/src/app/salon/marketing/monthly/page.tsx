      ...
      <div style={{ ...ui.card, marginBottom: 16 }}>
        ...
        {!budget && budgetErr && (
          <div style={{
            ...
            <span style={{ fontFamily: 'ui-monospace, monospace', color: 'var(--ink-warn)' }}>{budgetErr}</span>
          </div>
        )}
        {budget && (
          <div style={{
            ...
            {budget.ceilingCents ? (
              <>
                ...
                <div style={{ fontSize: 12, color: budget.feasible === 'no' ? 'var(--ink-bad)' : 'var(--c94a3b8)', lineHeight: 1.6, marginTop: 4 }}>
        ...
        <div key={c.platform} style={{ borderTop: '1px solid var(--line)', padding: '10px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 14, color: 'var(--ce2e8f0)', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {c.connected && !c.keyHint?.startsWith('LINKED:') && <span style={{ fontSize: 10.5, color: 'var(--ink-good)', border: '1px solid #22c55e', borderRadius: 999, padding: '1px 8px' }}>{T('đã kết nối', 'connected')}</span>}
        ...
        <div style={pv}><div style={pvL}>② {T('MANG VỀ', 'RESULTS')}</div>
          ...
          {(o.gbp?.bookings ?? 0) > 0 && (
            <div style={{ fontSize: 12, color: 'var(--c94a3b8)', marginTop: 8, borderTop: '1px solid var(--line)', paddingTop: 6 }}>
              {T('Từ Google Maps (đo đích danh)', 'From Google Maps (verified)')}: <b style={{ color: 'var(--ce2e8f0)' }}>{o.gbp!.bookings}</b> {T('lượt đặt', 'bookings')} · {o.gbp!.showed} {T('đã đến', 'showed')} · <b style={{ color: 'var(--ink-good)' }}>{money(o.gbp!.revenueCents)}</b>
            </div>
          )}
        </div>
      </div>
