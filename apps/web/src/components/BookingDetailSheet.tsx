...
const STATUS_COLOR: Record<string, string> = {
  ...
  ARRIVED: '#14b8a6', COMPLETED: '#a855f7', REJECTED: '#ef4444', CANCELLED: 'var(--c94a3b8)', NO_SHOW: '#ef4444',
};
...
export function BookingDetailSheet({ token, apptId, onClose, lang, L }: { token?: string | null; apptId: string; onClose: () => void; lang: string; L: (vi: string, en: string) => string }) {
  ...
  const statusColor = STATUS_COLOR[status] ?? 'var(--c94a3b8)';
  ...
  const card: CSSProperties = { width: '100%', maxWidth: 460, background: 'var(--c0f172a)', border: '1px solid #223047', borderRadius: isMobile ? '18px 18px 0 0' : 16, padding: '16px 18px 20px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 -8px 40px rgba(0,0,0,0.5)' };
  ...
  const content = (
    <div style={overlay} onClick={onClose}>
      <div style={card} onClick={(e) => e.stopPropagation()}>
        ...
        {err ? (
          <p style={{ color: 'var(--cf87171)', fontSize: 14, marginTop: 14 }}>{L('Không tải được chi tiết. Có thể lịch hẹn đã bị xoá.', 'Could not load details — the booking may have been removed.')}</p>
        ) : !d ? (
          <p style={{ color: 'var(--c64748b)', fontSize: 14, marginTop: 14 }}>{L('Đang tải chi tiết…', 'Loading details…')}</p>
        ) : (
          <>
