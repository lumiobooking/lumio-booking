...
function Inner() {
  ...
  if (loading && !data) return <section><h2 style={{ fontSize: 18 }}>{T('Báo cáo marketing', 'Marketing report')}</h2><p style={{ color: 'var(--c94a3b8)' }}>Loading…</p></section>;
    ...
    <section>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 8 }}>
        <div>
          ...
          <p style={{ color: 'var(--c94a3b8)', margin: '4px 0 0', fontSize: 13 }}>{T('Marketing mang lại bao nhiêu booking, bao nhiêu khách đến, doanh thu bao nhiêu — từ dữ liệu thật của tiệm.', 'How many bookings, showed-up customers and revenue marketing brought — from the salon’s real data.')}</p>
        </div>
      </div>
            ...
            {channels.map((c) => (
              <div key={c.key} style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--ce2e8f0)' }}>
      ...
      <div style={{ ...ui.card, marginBottom: 16 }}>
        ...
        {(data?.byCampaign ?? []).length === 0 ? (
          <p style={{ color: 'var(--c64748b)', fontSize: 13, margin: 0, lineHeight: 1.6 }}>
      ...
      <div style={{ border: '1px solid var(--c334155)', background: 'var(--c1e293b)', borderRadius: 10, padding: 12, fontSize: 12.5, color: 'var(--ca5b4fc)', lineHeight: 1.6 }}>
...
function Empty({ vi }: { vi: boolean }) {
  return <p style={{ color: 'var(--c64748b)', fontSize: 13, margin: 0 }}>{vi ? 'Chưa có dữ liệu trong kỳ này.' : 'No data in this period.'}</p>;
}
