...
export function SuggestionInbox({
  ...
  if (!feed.ready.length && !feed.waitingOnShop.length && !done.length) return null;

  const pages = Math.max(1, Math.ceil(feed.ready.length / PAGE));
  const p = Math.min(page, pages - 1);
  const slice = feed.ready.slice(p * PAGE, p * PAGE + PAGE);
  const donePages = Math.max(1, Math.ceil(done.length / PAGE));
  const dp = Math.min(donePage, donePages - 1);
  const doneSlice = done.slice(dp * PAGE, dp * PAGE + PAGE);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 9, alignItems: 'center', flexWrap: 'wrap', marginBottom: 3 }}>
        ...
        {feed.ready.length > 0 && (
          <span style={{ fontSize: 11.5, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: '#14532d', border: '1px solid #22c55e', color: '#86efac' }}>
            {feed.ready.length} {T('chờ mình dựng', 'waiting on us')}
          </span>
        )}
        {feed.driveFolderUrl && (
      ...
      <div style={{ fontSize: 11.5, color: 'var(--c64748b)', lineHeight: 1.55, marginBottom: 10 }}>
        {T('File gốc nằm trên Drive. "Dựng bài" chép một bản sang kho đăng bài trong 1 tháng rồi tự dọn; "Cất đi" ghi một dòng đã làm gì để sau còn tra.',
           'Originals live on Drive. "Make a post" stages a copy on the hosting for a month; "Put away" keeps one line on what was done.')}
      </div>
      {err && <div style={{ fontSize: 12.5, color: 'var(--cfca5a5)', marginBottom: 8 }}>{err}</div>}

      {/* ---- waiting on us: the grid ---- */}
      {slice.length > 0 && (
        <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))' }}>
          {slice.map((s) => (
            <div key={s.id} style={{ background: 'var(--c0f172a)', border: '1px solid #166534', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                {closing === s.id ? (
                  <div>
                    <input
                      ...
                      onKeyDown={(e) => { if (e.key === 'Enter') void putAway(s.id); if (e.key === 'Escape') setClosing(null); }}
                      placeholder={T('Đã làm gì? — vd: dựng reel thứ 3', 'What was done? — e.g. Tuesday reel')}
                      style={inp}
                    />
                    <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                      <button onClick={() => putAway(s.id)} disabled={busy === s.id} style={{ ...btn, flex: 1, background: '#6366f1', borderColor: '#6366f1', color: '#fff' }}>✓ {T('Cất đi', 'Put away')}</button>
                      <button onClick={() => { setClosing(null); setNote(''); }} style={btn}>{T('Huỷ', 'Cancel')}</button>
                    </div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', gap: 6, marginTop: 'auto' }}>
                    ...
                    <button onClick={() => { setClosing(s.id); setNote(''); }} title={T('Đã xử lý — ghi một dòng rồi cất', 'Handled — note it and put away')} style={btn}>✓</button>
                  </div>
                )}
              </div>
            </div>
      ...
      {pages > 1 && <Pager page={p} pages={pages} onPage={setPage} vi={vi} />}
        ...
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--c334155)' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {feed.waitingOnShop.map((s) => (
              <span key={s.id} title={s.title} style={{ fontSize: 12, color: 'var(--c94a3b8)', background: 'var(--c0f172a)', border: '1px solid var(--c334155)', borderRadius: 8, padding: '4px 9px', maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--c334155)' }}>
          ...
          {showDone && (
            <>
              ...
              {donePages > 1 && <Pager page={dp} pages={donePages} onPage={setDonePage} vi={vi} />}
            </>
          )}
        </div>
      )}
