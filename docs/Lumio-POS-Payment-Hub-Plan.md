...
- **Nối POS quầy (A):** màn `/salon/pos` khi trả **CARD** tự đẩy tiền xuống reader WiFi đã kết nối (overlay "chờ khách quẹt"), quẹt xong mới ghi bill; tự chọn reader ONLINE. Tiệm chưa kết nối → CARD vẫn thủ công như cũ (không phá gì).
- **Mobile POS — nền (B):** thư mục `mobile/` = app **React Native + Stripe Terminal SDK** (Bluetooth + **Tap to Pay**): đăng nhập → connection-token → charge on-device. Backend đã trả `clientSecret` cho SDK. **Là nền chạy được, CHƯA store-ready** (cần dev build native + entitlement Apple/Google + QA — xem `mobile/README.md`).
