...
export default function PosDisplayPage() {
  ...
  return (
    <div style={{ ...page, ...(imgWelcome ? { padding: 0 } : null) }}>
      ...
      <div style={{ position: 'fixed', bottom: 6, right: 10, fontSize: 10, color: 'var(--ccbd5e1)', pointerEvents: 'none', userSelect: 'none' }}>pos-display v4</div>
    </div>
  );
}
...
const CHECKIN_CSS = `
  ...
  .lumio-yscroll::-webkit-scrollbar-thumb{background:var(--ccbd5e1);border-radius:5px}
...
function CheckInScreen({ st, salonName, logo, send }: {
    ...
    return (
      <div style={{ ...page, alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        <div style={{ maxWidth: 760 }}>
          ...
          <div style={{ fontSize: 'clamp(14px, 1.8vw, 18px)', color: 'var(--c94a3b8)', marginTop: 22 }}>
  ...
  const input: React.CSSProperties = {
    ...
    border: '1px solid var(--ccbd5e1)', background: '#fff', color: 'var(--c0f172a)',
  ...
  const chip = (on: boolean): React.CSSProperties => ({
    border: `2px solid ${on ? '#4f46e5' : 'var(--ce2e8f0)'}`, background: on ? '#4f46e5' : '#fff',
    color: on ? '#fff' : 'var(--c475569)', borderRadius: 999, padding: '13px 20px',
  ...
  return (
    <div style={{ ...page, padding: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        borderBottom: '1px solid var(--ce2e8f0)', flexShrink: 0, background: '#fff',
      }}>
        {logo
          // eslint-disable-next-line @next/next/no-img-element
            ...
            return (
              <button key={sv.id} onClick={() => tap(sv.id)}
                style={{
                  ...
                  border: `3px solid ${on ? '#4f46e5' : 'var(--ce2e8f0)'}`,
      ...
      <div style={{
        ...
        borderTop: '1px solid var(--ce2e8f0)', background: 'var(--cf8fafc)',
      }}>
        <span style={{ fontSize: 16.5, color: 'var(--c475569)' }}>
          {picked.length > 0
...
const lbl: React.CSSProperties = { display: 'block', fontSize: 15, color: 'var(--c475569)', marginBottom: 7, fontWeight: 600 };
